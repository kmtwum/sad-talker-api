...
class AnimateFromCoeff_PIRender():
    ...
    def __init__(self, sad_talker_path, device):

        opt = Config(sad_talker_path['pirender_yaml_path'], None, is_train=False)
        opt.device = device
        self.net_G_ema = FaceGenerator(**opt.gen.param).to(opt.device)
        checkpoint_path = sad_talker_path['pirender_checkpoint']
